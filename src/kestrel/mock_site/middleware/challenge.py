...
or assets are contacted. HARD-mode intermittent rolling and the CSRF
service land in section 08; this section gates EASY (no-op) and MEDIUM
(redirect when clearance is missing or stale).
"""
...
class ChallengeMiddleware(BaseHTTPMiddleware):
    ...
    `challenge_expired` for a recognised but stale token). The HARD
    intermittent roll lands in section 08.
    """
